...
export const LineChartComponent = ({
  ...
  return (
    <Glass rounded="3xl" className={`p-6 ${className}`}>
      {title && (
        <div className="mb-5">
          <p className="font-sans-display text-[10px] uppercase tracking-[0.32em] text-cream-400">Trend</p>
...
export const BarChartComponent = ({
  ...
  return (
    <Glass rounded="3xl" className={`p-6 ${className}`}>
      {title && (
        <div className="mb-5">
          <p className="font-sans-display text-[10px] uppercase tracking-[0.32em] text-cream-400">Comparison</p>
          <h3 className="display mt-1 text-2xl text-cream-50">{title}</h3>
        </div>
      )}
      <div className="h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            <XAxis dataKey={xAxisKey} tick={axisStyle} stroke="rgba(245,239,226,0.16)" />
            <YAxis tick={axisStyle} stroke="rgba(245,239,226,0.16)" />
            <Tooltip contentStyle={tooltipStyle} labelStyle={labelStyle} cursor={{ fill: 'rgba(231, 111, 46, 0.08)' }} />
            <defs>
...
export const PieChartComponent = ({
  ...
  return (
    <Glass rounded="3xl" className={`p-6 ${className}`}>
      {title && (
        <div className="mb-5">
          <p className="font-sans-display text-[10px] uppercase tracking-[0.32em] text-cream-400">Distribution</p>
          <h3 className="display mt-1 text-2xl text-cream-50">{title}</h3>
        </div>
      )}
      <div className="h-64 w-full">
...
export const RadarChartComponent = ({
  ...
  return (
    <Glass rounded="3xl" className={`p-6 ${className}`}>
      {title && (
        <div className="mb-5">
          <p className="font-sans-display text-[10px] uppercase tracking-[0.32em] text-cream-400">Profile</p>
          <h3 className="display mt-1 text-2xl text-cream-50">{title}</h3>
        </div>
      )}
      <div className="h-64 w-full">
